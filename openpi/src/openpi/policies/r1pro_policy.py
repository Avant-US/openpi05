"""R1 Pro robot policy adapter for OpenPI.

Maps R1 Pro data fields to pi0/pi0.5 model expected format.

State/Action: 20-dim = left_arm(7) + right_arm(7) + left_gripper(1) + right_gripper(1) + torso(4)
...
def make_r1pro_example() -> dict:
    """Creates a random input example for the R1 Pro policy."""
    return {
...
class R1ProInputs(transforms.DataTransformFn):
    """Converts R1 Pro inputs to model expected format.
...
class R1ProOutputs(transforms.DataTransformFn):
    """Converts model outputs back to R1 Pro action format.

    Returns only the first 20 dims (strips zero-padding).
    """
